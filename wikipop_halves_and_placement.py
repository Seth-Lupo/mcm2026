"""
wikipop_halves_and_placement.py

EXTENDS the Wikipedia pageviews popularity script with:
1) Season HALF splits:
   - first half of the season window
   - second half of the season window
   For each half we compute: total, mean, peak, std of daily pageviews.
2) Placement column appended to the output CSV:
   - pulled from your DWTS dataset column `placement`
   - one value per (season, celebrity_name)

Output:
- wiki_popularity_by_season.csv  (now includes half-season metrics + placement)

Inputs:
- 2026_MCM_Problem_C_Data.csv  (must include: season, celebrity_name, placement)
- dates.csv                   (must include: season, week, date)

Install:
  pip install pandas requests numpy
Run:
  python wikipop_halves_and_placement.py

Notes on "first half" vs "second half":
- We split by *days* across the season date range [start_date, end_date].
- Let N = number of days in the inclusive season window.
- first half = first ceil(N/2) days
- second half = remaining floor(N/2) days
This guarantees:
- all season days are included exactly once
...
# ------------------------------------
SEASONS_TO_RUN: Optional[set[int]] = {33}  # e.g., {27, 28}
...
def get_placement_by_season(data_df: pd.DataFrame) -> Dict[tuple[int, str], Optional[int]]:
    """
    Build a mapping (season, celebrity_name) -> placement (int) from your dataset.

    Assumptions:
    - `placement` is constant for a contestant across all rows in that season.
    - If multiple values exist due to data issues, we take the most frequent non-null value.
    """
    placements = {}
    # group by season and celebrity
...
def fetch_pageviews_daily(title: str, start_dt: date, end_dt: date) -> List[dict]:
    """
    Fetch daily pageviews for one article in [start_dt, end_dt] inclusive.
...
def split_season_into_halves(start_date: date, end_date: date) -> Tuple[Tuple[date, date], Tuple[date, date]]:
    """
    Split an inclusive date range [start_date, end_date] into two halves by days.

    Let N = number of days inclusive.
...
def main() -> None:
    ...
    # Build placement mapping once (so we don't recompute in inner loops)
    ...
    for w in season_windows:
        ...
        for name in cast:
            ...
            # Half 2 views (may be empty if season is 1-day long, but that won't happen here)
    ...
    # Put placement at the end of the CSV (as you requested).
    # We'll reorder columns so placement is the last column.
    if "placement" in out_df.columns:
        cols = [c for c in out_df.columns if c != "placement"] + ["placement"]
        out_df = out_df[cols]
